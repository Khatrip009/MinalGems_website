...
type RegisterResult =
  | { ok: true }
  | { ok: false; reason: "ssr" | "unsupported" | "permission_denied" | "not_configured" | "error" };

/** GET /api/push/vapid → public VAPID key
 *
 * NOTE: do NOT prefix with /api here, client.ts already handles base URL,
 * otherwise you get http://localhost:4500/api/api/push/vapid
 */
...
/**
 * Registers browser push and saves subscription in DB.
 * Uses:
 *   GET  /api/push/vapid
...
export async function registerPushForCurrentVisitor(): Promise<RegisterResult> {
  ...
  // IMPORTANT: do NOT JSON.stringify body – client.ts handles it
