...
export default function Cart() {
  ...
  // ✅ FIX 1: Use backend's `total` field. If it's 0 or missing, calculate from items.
  const subtotal =
    Number(cart?.total) ||
          ...
          <>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {cartItems.map((item) => {
                    ...
                    const imageUrl = item.primary_image || '/placeholder.jpg';
                    ...
                    return (
                      <tr key={itemId} className="border-b border-gold-100">
                        ...
                        <td className="py-5 text-right text-charcoal">
                        ...
                        <td className="py-5 text-right font-medium text-charcoal">
                  ...
                  <div key={itemId} className="border border-gold-200 bg-white p-4 rounded-sm">
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        <Link to={`/product/${productSlug}`} className="font-serif text-charcoal">
                          {productTitle}
                        </Link>
                        <p className="text-gold-600 mt-1">₹{price.toLocaleString('en-IN')}</p>
                        <div className="flex items-center justify-between mt-3">
                          <div className="flex items-center gap-1">
                            <button
                              onClick={() => handleQuantityChange(itemId, qty - 1)}
                              disabled={updating === itemId}
                              className="w-7 h-7 border border-gold-300 text-gold-600"
                            >
                              −
                            </button>
            ...
            <div className="mt-12 border-t border-gold-200 pt-8 flex flex-col items-end">
              <div className="w-full max-w-sm space-y-4">
                ...
                <Link
                  to="/shop"
                  className="block text-center text-sm text-gold-600 hover:text-gold-700 underline"
                >
                  Continue Shopping
                </Link>
              </div>
